...
const mapsymbols = [...new Array(4052)].reduce((a, v, c) => {
  if (
    [
      92, 173, 847, 888, 889, 907, 909, 930, 1328, 1367, 1368, 1376, 1416, 1419,
      1420, 1421, 1424, 1565, 1867, 1868, 2081, 2082, 2094, 2095, 2111, 2140,
      2141, 2445, 2446, 2449, 2450, 2473, 2481, 2490, 2491, 2501, 2502, 2505,
      2506, 2526, 2532, 2533, 2564, 2577, 2578, 2601, 2609, 2612, 2615, 2618,
      2619, 2621, 2633, 2634, 2638, 2639, 2640, 2653, 2692, 2702, 2706, 2729,
      2737, 2740, 2746, 2747, 2758, 2762, 2766, 2767, 2788, 2789, 2820, 2829,
      2830, 2833, 2834, 2857, 2865, 2868, 2874, 2875, 2885, 2886, 2889, 2890,
      2910, 2916, 2917, 2948, 2955, 2956, 2957, 2961, 2966, 2967, 2968, 2971,
      2973, 2976, 2977, 2978, 2981, 2982, 2983, 2987, 2988, 2989,
    ].includes(c)
  ) {
    return a;
  }
...
function verifyTiledMap(mapObj) {
  ...
  if (mapObj.orientation !== "orthogonal") {
    throw new Error(
      `Map is ${mapObj.orientation}. Only orthogonal is currently supported.`
    );
  }
  if (mapObj.renderorder !== "right-down") {
    throw new Error(
      `Map is ${mapObj.renderorder}. Only right-down is currently supported.`
    );
  }
}
...
const tiledMapPlugin = (k) => {
  ...
  return {
    ...
    async loadTiledMap(jsonUrl, imageUrl = "") {
      return fetch(jsonUrl)
        ...
        .then(async (mapObj) => {
          ...
          for (let tileset of mapObj.tilesets) {
            if (tileset.margin !== 0) {
              console.error(
                `${tileset.name} uses margins. That is currently unsupported. Skipping.`
              );
              continue;
            }
            if (tileset.spacing !== 0) {
              console.error(
                `${tileset.name} uses spacing. That is currently unsupported. Skipping.`
              );
              continue;
            }

            // console.log("image width", tileset.tilewidth);
            // console.log("image height", tileset.tileheight);

            const sliceX = tileset.imagewidth / tileset.tilewidth;
            const sliceY = tileset.imageheight / tileset.tileheight;

            // console.log("SLICE X", sliceX);
            // console.log("SLICE X", sliceY);
            // console.log("loction", imageUrl);

            map.sprites.push(
              await k.loadSprite(tileset.name, `${imageUrl}`, {
                sliceX,
                sliceY,
              })
            );
            ...
            for (
              let frame = tileset.firstgid;
              frame < tileset.firstgid + tileset.tilecount;
              frame++
            ) {
              map.key[mapsymbols[frame]] = [
          ...
          for (let layer of mapObj.layers) {
            if (layer.type === "tilelayer" && layer.visible) {
              ...
              const mapArray = [...new Array(width)].map(() =>
                new Array(height + 1).fill(" ")
              );
